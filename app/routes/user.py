    ...
    if request.method == 'POST':
        ...
        if User.query.filter_by(username=username).first():
            ...
            user = User(
                ...
                name=name,
                role=role,
                can_view_order=int(request.form.getlist('can_view_order')[-1] if request.form.getlist('can_view_order') else 1),
                can_deliver=int(request.form.getlist('can_deliver')[-1] if request.form.getlist('can_deliver') else 0),
                can_refund=int(request.form.getlist('can_refund')[-1] if request.form.getlist('can_refund') else 0),
            )
...
def user_edit(user_id):
    ...
    if request.method == 'POST':
        ...
        # getlist()取最后一个值，兼容hidden+checkbox双值提交（get()只取第一个会永远拿到hidden的0）
        user.can_view_order = int(request.form.getlist('can_view_order')[-1] if request.form.getlist('can_view_order') else 1)
        user.can_deliver = int(request.form.getlist('can_deliver')[-1] if request.form.getlist('can_deliver') else 0)
        user.can_refund = int(request.form.getlist('can_refund')[-1] if request.form.getlist('can_refund') else 0)
        user.is_active_flag = int(request.form.getlist('is_active')[-1] if request.form.getlist('is_active') else 1)
