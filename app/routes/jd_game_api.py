...
@jd_game_api_bp.route('/direct-receive', methods=['POST'])
def game_direct_order():
    ...
    try:
        ...
        _log = ApiLog(
            api_type='game_direct_inbound',
            request_method=request.method,
            request_url=request.url,
            request_headers=str(dict(request.headers))[:2000],
            request_body=_json.dumps(raw, ensure_ascii=False)[:4000],
            response_status=0,
...
def game_direct_query():
    ...
    # 直充状态映射：JD文档 0=充值成功 1=充值中 2=充值失败
    jd_status_map = {0: 1, 1: 1, 2: 0, 3: 2, 4: 2, 5: 2}
...
@jd_game_api_bp.route('/card-receive', methods=['POST'])
def game_card_order():
    """游戏点卡 - 卡密接单接口
...
def game_card_query():
    ...
    if order.order_status == 2 and order.card_info_parsed:
        ...
        for card in raw_cards:
            ...
            card_pass = card.get('cardpass') or card.get('cardPass') or card.get('cardPwd') or card.get('card_pwd') or ''
