...
def shop_delete(shop_id):
    ...
    if shop:
        ...
        db.session.commit()
        _log_operation('delete_shop', 'shop', sid, f'删除店铺: {shop_name}')
        flash('店铺已删除', 'success')
    return redirect(url_for('shop.shop_list'))
...
def _fill_shop_fields(shop, form):
    ...
    shop.jd_union_appkey = form.get('jd_union_appkey', '').strip() or None
    shop.jd_union_secretkey = form.get('jd_union_secretkey', '').strip() or None
    agiso_app_id = form.get('agiso_app_id', '').strip()
    agiso_app_secret = form.get('agiso_app_secret', '').strip()
    agiso_access_token = form.get('agiso_access_token', '').strip()
    if agiso_app_id:
        shop.agiso_app_id = agiso_app_id
    if agiso_app_secret:
        shop.agiso_app_secret = agiso_app_secret
    if agiso_access_token:
        shop.agiso_access_token = agiso_access_token
    shop.agiso_enabled = 1 if shop.agiso_access_token else 0
